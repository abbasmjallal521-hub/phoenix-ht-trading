...
async function sendTestText(textToSend){
  var text = textToSend || $('customText').value;
  if(!text) return;
  try {
    var res = await fetch('/api/signals/process', {
      method: 'POST',
...
function render(){
  var active = signals.filter(function(s) { return s.status === 'ACTIVE' || s.status === 'PENDING'; });
  var closed = signals.filter(function(s) { return s.status !== 'ACTIVE' && s.status !== 'PENDING'; });
...
function createCard(signal){
  ...
  var updates = (signal.profitUpdates || []).map(function(x) {
    return '<div class="pip-badge">💰 +' + x.pips + ' PIP</div>';
  }).join('');

  return '<div class="signal-card">' +
    '<div class="signal-header">' +
      '<div class="direction-tag ' + dirClass + '">' + dirIcon + ' ' + signal.symbol + '</div>' +
      '<span class="status-badge ' + (signal.status === 'ACTIVE' ? 'active' : '') + '">' + (statusMap[signal.status] || signal.status) + '</span>' +
    '</div>' +
    '<div class="grid-details">' +
      '<div class="cell"><span>Entry</span><b>' + val(signal.entry) + '</b></div>' +
      '<div class="cell sl"><span>Stop Loss</span><b>' + val(signal.stop) + '</b></div>' +
      '<div class="cell tp"><span>Target</span><b>' + val(signal.target) + '</b></div>' +
    '</div>' +
